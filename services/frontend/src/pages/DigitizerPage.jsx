...
function StitchSvgPreview({ stitches, colors = [] }) {
  ...
  const segments = [];
  let fill = [], satin = [], colorIdx = 0, px = null, py = null;

  const flush = () => {
    if (fill.length || satin.length) segments.push({ fill: fill.join(''), satin: satin.join(''), colorIdx });
    fill = []; satin = [];
  };

  for (const s of stitches) {
    if (s.type === 'end') break;
    if (s.type === 'color_change') { flush(); colorIdx++; px = s.x; py = s.y; continue; }
    ...
    if (px !== null) {
      ...
      (isSatin ? satin : fill).push(`M${px},${py}L${s.x},${s.y}`);
    }
    px = s.x; py = s.y;
  }
  flush();
  ...
  const getColor = (idx) => colors[idx]?.hex || DEFAULT_THREAD_COLORS[idx % DEFAULT_THREAD_COLORS.length];
  const pad = 40;
  const vw = maxX - minX + pad * 2, vh = maxY - minY + pad * 2;

  return (
    <svg viewBox={`${minX - pad} ${minY - pad} ${vw} ${vh}`}
      style={{ width: '100%', height: '100%', display: 'block', background: '#111111', borderRadius: 10 }}
      preserveAspectRatio="xMidYMid meet">
      {segments.map((seg, i) => {
        const color = getColor(seg.colorIdx);
        return (
          <g key={i}>
            {seg.fill  && <path d={seg.fill}  fill="none" stroke={color} strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round" opacity="1" />}
            {seg.satin && <path d={seg.satin} fill="none" stroke={color} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" opacity="1" />}
          </g>
        );
      })}
    </svg>
          ...
          {result && (
            <div className="fade-up" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {result.colors?.length > 0 && (
                <div>
                  <div className="section-header">Thread Colors</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {result.colors.map((c, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 11, padding: '4px 0' }}>
                        <span style={{
                          width: 13, height: 13, borderRadius: 3, flexShrink: 0,
                          background: c.hex, border: '1px solid rgba(255,255,255,0.18)',
                        }} />
                        <span style={{ color: 'var(--dim)', fontFamily: 'var(--mono)', minWidth: 14 }}>{i + 1}</span>
                        <span style={{ color: 'var(--muted)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
                        <span style={{ color: 'var(--dim)', fontFamily: 'var(--mono)', fontSize: 10, whiteSpace: 'nowrap' }}>Madeira {c.code}</span>
                        <span style={{ color: 'var(--accent-light)', fontFamily: 'var(--mono)', minWidth: 52, textAlign: 'right', whiteSpace: 'nowrap' }}>
                          {c.stitchCount?.toLocaleString()} st
                        </span>
                      </div>
                    ))}
                  </div>
        ...
        {!result && !loading && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, color: 'var(--muted)', pointerEvents: 'none' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13 }}>{file ? 'Click Generate Stitches to begin' : 'Upload a logo and click Generate Stitches'}</div>
            </div>
          </div>
        )}
        {(result || loading) && (
          <div style={{ flex: 1, minHeight: 0, display: 'grid', gridTemplateRows: 'auto minmax(0, 1fr)', gap: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <h1 style={{ marginBottom: 3 }}>Stitch Preview</h1>
                <p style={{ fontSize: 12 }}>Actual stitch paths — fill stitches + satin border per color layer.</p>
              </div>
              {result && (
            ...
            <div style={{ minHeight: 0, borderRadius: 10, overflow: 'hidden', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#111111' }}>
              {loading && <span style={{ color: 'var(--muted)' }}>Generating stitches…</span>}
              {!loading && result && <StitchSvgPreview stitches={result.stitches} colors={result.colors} />}
            </div>
          </div>
        )}
      </div>
